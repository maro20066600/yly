...
drawButton.addEventListener('click', () => {
    ...
    setTimeout(() => {
        ...
        function animateWandToNextCard() {
            ...
            // Create sparkle at wand tip
            const sparkle = document.createElement('div');
            sparkle.className = 'wand-sparkle';
            sparkle.style.left = `${card.x}px`;
            sparkle.style.top = `${card.y}px`;
        ...
        function animateToSelectedCards() {
            ...
            function selectNextCard() {
                ...
                // Add special sparkle effect around the card
                for (let i = 0; i < 12; i++) {
        ...
        function flipSelectedCards() {
            ...
            // (This is the original card flipping code, but we'll remove the rotating animation since we did that already)
            positions.forEach((pos, index) => {
                setTimeout(() => {
                    const card = cards[pos];
                ...
                }, index * 2000); // 2 second delay between cards (reduced from original 3 seconds)
            });
        }
